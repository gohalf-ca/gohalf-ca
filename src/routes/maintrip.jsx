...
export default function MainTripsPage() {
  ...
  async function refreshedTrips  () {
    let response =  await fetch(`${import.meta.env.VITE_Backend_URL}/alltrips/${clerk_ID}`)
  ...
  const deleteTrip = async (tripID) => {
    try {
      ...
      Console.log("Error occured delete value " + error);
    }
  };
          ...
          {trips.map((trip, index) => (
            ...
            >
              <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-center items-center">
                ...
                <Link
                  to={`/tripDetails/${trip.trip_id}`} // Переход к детальной странице трипа
